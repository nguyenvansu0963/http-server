...
double gettimeofday_sec(void)
{
	...
	return tv.tv_sec + tv.tv_usec * 1e-6;	//tv_secが秒　tv_usecはuがマイクロという意味として使っており、マイクロ秒を整数として取り扱っているので1e-6している
}
...
int exp1_do_talk(int sock)
{
	...
	if (FD_ISSET(sock, &fds) != 0)
	{
		...
		if (ret > 0)
		{
			int size = write(1, buf, ret);	//ファイルディスクリプタ1番は標準出力用のfd=が1だから
			if (size == -1)
			{
				perror("error in write: ");
			}
